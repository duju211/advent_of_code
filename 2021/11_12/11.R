...
erg_vec <- vector(mode = "list", length = 100)
df_grid_today <- df_grid_tidy
for (i in seq_along(erg_vec)) {
  erg_vec[[i]] <- sim_flashes(df_grid_today)
  df_grid_today <- erg_vec[[i]]
}

bind_rows(erg_vec) |>
  summarise(erg = sum(value == 0))
